...
{
    _station_controller = std::make_shared<PIDController>();
    _speed_controller = std::make_shared<PIDController>();
    // 设置控制器参数
    _station_controller->set_controller(1.0, 0.1, 1.0);
    _speed_controller->set_controller(4.0, 0.1, 1.0);
}
     ...
     ControlCMD & cmd)
{
    ...
    {
        cmd.brake = 0.0;
        cmd.throttle = std::max(std::min(acceleration_cmd * 0.5, 1.0),0.0);
    }
    else
    {
        cmd.throttle = 0.0;
        // 乘这个0.1是为了避免刹车太硬,改善控制效果
        cmd.brake = std::max(std::min(-acceleration_cmd * 0.09, 1.0),0.0);
    }
