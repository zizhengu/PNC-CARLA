...
public:
   ...
   */
  // 设置样条的域（定义域 vec_domain），表示样条的每个段的起点和终点。
  ...
  ErrorType evaluate(const decimal_t s, const int d, Vecf<N_DIM> *ret) const
  {
    ...
    else
    {
      ...
      // 缩放因子是为了在不同参数化区间下正确计算曲线的导数值，确保物理意义和数学模型的一致性。
      Vecf<N_DIM> result =
  ...
  void print_ctrl_pts() const
  {
    std::cout << "Control Points:" << std::endl;
    for (size_t i = 0; i < ctrl_pts_.size(); ++i)
    {
      std::cout << "Segment " << i << ":" << std::endl;
      for (int row = 0; row < ctrl_pts_[i].rows(); ++row)
      {
        std::cout << "  Control Point " << row << ": "
                  << ctrl_pts_[i].row(row) << std::endl;
      }
    }
  }
...
public:
   ...
   */
  // 在代码中，hessian 矩阵直接提供了预计算值，用于五阶贝塞尔曲线的三阶导数（jerk）
  // J = P.T * H * P，也即计算jerk代价的中间项
  // P是控制点向量 H 是三阶导数的 Hessian 矩阵
